...
struct group_join_t
{
    ...
    struct inner_selector
    {
        InnerKeySelector is;

        inner_selector(InnerKeySelector is) : is(is)
        {}
    ...
    template<class InnerKeySelector>
    static auto make_inner_selector(InnerKeySelector is) LINQ_RETURNS(inner_selector<InnerKeySelector>(is));
    ...
    struct selector
    {
        ...
        template<class Range>
        selector(Range && r, OuterKeySelector os, ResultKeySelector rs)
    ...
    static auto make_selector(Range && r, OuterKeySelector os, ResultKeySelector rs) LINQ_RETURNS
    (selector<typename boost::range_value<R>, OuterKeySelector, ResultKeySelector >
    (
        r, os, rs
    ));
    ...
    (
        outer | linq::select(make_selector(inner | linq::select(make_inner_selector(is)), outer_key_selector, result_selector))
    );
};
}
namespace {
